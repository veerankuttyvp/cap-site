 ...
 */
class MenteeSearchController extends AbstractRestfulController
{
    /**
     * @var ModuleOptions
     */
   ...
   #     return new JsonModel(array('data' => $user));
    ...
    public function create($data)
    {
        ...
        $entityManager = $this->getEntityManager();
        $user = $entityManager->createQuery("SELECT u.id, u.firstName, s.name  as status FROM CsnUser\Entity\Customer u JOIN u.status s WHERE u.role = 6 and u.firstName LIKE '%$data[key]%'")->getResult(\Doctrine\ORM\Query::HYDRATE_OBJECT);
        return new JsonModel(array('data' => $user));
